...
class SvgOnMatplotlib(object):
    def __init__(self, fig, ax, svg, driver_path=None):
        ...
        self.min_x, self.min_y, self.win_width, self.win_height = list(
            map(float, self.soup.find('svg')['viewbox'].split()))
        self.real_width, self.real_height = [float(self.soup.find('svg')['width'].replace('pt', '').replace("px", '') if 'width' in self.soup.find('svg').attrs else 1000),
                                             float(self.soup.find('svg')['height'].replace('pt', '').replace("px", '') if 'height' in self.soup.find('svg').attrs else 1000)]
        self.txt_lst = []
        self.clip_path = {}
        self.setup_plot()
    ...
    def get_absolute_path(self, path):
        ...
        open('converter.html', 'w').write(re.sub(r"'(.+)'", "'"+re.sub(r'(\d)\s(\d)',
                                                                       r'\1,\2', path.replace('\n', '')).replace('z', '')+"'", converter))
    ...
    def get_bevier_curve(self, p0, p1, p2, p3):
        # cubic bezier fomula
        def b(t): return p0*(1-t)**2 * (1-t) + 3*p1 * \
            (1-t)**2*t + 3*p2*(1-t)*t**2 + p3*t**2 * t
        return [b(i) for i in np.arange(0.0, 1.1, 0.1)]

    def build_path(self, path):
        data = self.get_absolute_path(path)
        data = [i.strip() for i in re.split('(M|C|L|S|H|V|Q)',
                                            re.sub(r'\d(\s)\d', ',', data))[1:]]
        ...
        for i in range(len(data)):
            ...
            # straight line [[float, float]]
                ...
                data[i][1] = [sum([[float(l) for l in [j[0]]+['-'+k for k in j[1:]] if l] for j in i], []) for i in [
                    [[k for k in j.split('-')] for j in i.strip().split(',')] for i in data[i][1].split('L') if i]]

            # continued bezier curve [[float, float], [float, float]]
            elif data[i][0] == 'S':
                data[i][1] = [sum([[float(l) for l in [j[0]]+['-'+k for k in j[1:]] if l] for j in i], []) for i in [
                    [[k for k in j.split('-')] for j in i.strip().split(',')] for i in data[i][1].split('S') if i]][0]
                data[i][1] = [data[i][1][j:j+2]
    ...
    def get_clip(self, item):
        item = item.find(['path', 'polyline', 'rect',
                          'polygon', 'line', 'text'])
        if item.name == 'path':
            patch = self.Path(item)

        elif item.name == 'rect':
            x, y, width, height = float(item['x']), float(
                item['y']), float(item['width']), float(item['height'])
            node = [(x, y), (x+width, y), (x+width, y+height),
                    (x, y+height), (x, y)]
            node = [tuple(reversed(i)) for i in node]
            patch = mpatches.PathPatch(
                mpath.Path(node), facecolor=(0, 0, 0, 0))
    ...
    def Path(self, item):
        ...
        width = item['stroke-width'].replace('px',
                                             '') if 'stroke-width' in item.attrs else 1
        fill = item['fill'] if 'fill' in item.attrs else 'alpha' if 'fill' in item.attrs and item['fill'] == 'none' else None
        if 'style' in item.attrs:
            style = item['style']
            if 'fill' in style:
                fill = re.search(r'fill:(.*?);', style).group(1)
            if 'stroke' in style:
                color = re.search(r'stroke:(.*?);', style).group(1)
            if 'stroke-width' in style:
                width = re.search(
                    r'width:(.*?);', style).group(1).replace('px', '')
        ...
        for i in range(len(data)):
            ...
            if data[i][0] == 'S':
                p0 = Vec2D(*data[i-1][1][-1])
                p1 = Vec2D(2*data[i-1][1][-1][0]-data[i-1][1][-2]
                           [0], 2*data[i-1][1][-1][1]-data[i-1][1][-2][1])
                p2 = Vec2D(*data[i][1][0])
                p3 = Vec2D(*data[i][1][1])
                points = self.get_bevier_curve(p0, p1, p2, p3)
                node += points
        ...
        node = [tuple([i[0], self.win_height - i[1]]) for i in node]
        path = mpath.Path(node, code)
        patch = mpatches.PathPatch(path, fill=True if fill else False, facecolor=(
            0, 0, 0, 0) if fill == 'alpha' else fill, ec=color if color else (0, 0, 0, 0), lw=float(width))
        if 'clip-path' in item.attrs:
            current_clip = item['clip-path']
            if 'url' in current_clip:
                current_clip = self.clip_path[re.search(
                    r'url\(#(.*?)\)', current_clip).group(1)]
    ...
    def Polyline(self, item):
        ...
        node = [[float(j) for j in i.split(',')]
                for i in item['points'].split() if i]
        code = [mpath.Path.MOVETO]+[mpath.Path.LINETO]*(len(node)-1)
        node = [tuple([i[0], self.win_height - i[1]]) for i in node]
        path = mpath.Path(node, code)
        patch = mpatches.PathPatch(path, facecolor=fill if fill else (
            0, 0, 0, 0), ec=color if color else (0, 0, 0, 0), lw=float(width))
    ...
    def Rect(self, item):
        ...
        if 'style' in item.attrs:
            ...
            if 'stroke-width' in style:
                stroke_width = int(
                    re.search(r'width:(.*?);', style).group(1).replace('px', ''))
            if re.match(r'(;|^)opacity:(.*?)(;|$)', style):
                alpha = float(
                    re.search(r'(;|^)opacity:(.*?)(;|$)', style).group(2))
        code = [mpath.Path.MOVETO]+[mpath.Path.LINETO]*4
        node = [(x, y), (x+width, y), (x+width, y+height),
                (x, y+height), (x, y)]
        node = [tuple([i[0], self.win_height - i[1]]) for i in node]
        path = mpath.Path(node, code)
        patch = mpatches.PathPatch(path, fill=True if fill else False, facecolor=fill if fill else None, ec=color if color else (
            0, 0, 0, 0), lw=float(stroke_width), alpha=alpha)
    ...
    def Line(self, item):
        p1, p2 = [float(item['x1']), float(item['x2'])], [
            float(item['y1']), float(item['y2'])]
        color = item['stroke'] if 'stroke' in item.attrs else None
        width = item['stroke-width'] if 'stroke-width' in item.attrs else 1
    ...
    def Text(self, item):
        x, y = [float(item['x']) if 'x' in item.attrs else 0,
                float(item['y']) if 'y' in item.attrs else 0]
        ...
        if transform:
            if 'matrix' in transform:
                matrix = self.decomposeMatrix(dict(zip('abcdef', list(
                    map(float, re.search(r'matrix\((.+)\)', transform).group(1).split())))))
        ...
        for item in self.soup.findAll(['path', 'polyline', 'rect', 'polygon', 'line', 'text']):
            try:
                if item.name == 'path':
                    ...
                    self.ax.plot(p1, self.win_height - p2, color=color if color else (
                        0, 0, 0, 0), lw=float(width))
                    plt.pause(0.001)

                if item.name == 'text':
                    x, y, text, font_size, matrix = self.Text(item)
                    self.txt_lst.append(self.ax.text(x, self.win_height - y, text, rotation=360-matrix['rotation'] if matrix else 0, fontsize=2, ha='right', va=(
                        'top' if matrix['rotation']+90.0 in [0, 180] else 'bottom') if matrix else 'top'))
                    plt.pause(0.001)
            except:
                pass
...
if __name__ == "__main__":
    ...
    fig = figure_pz(figsize=(15, 5), dpi=100)
    fig.tight_layout()
    '''
	count = 1
